    ...
    if not is_admin_logged():
        ...
        if tab_docs.open:
            with tab_docs:
                ...
                def load_doc_dates():
                    if os.path.exists(DOC_DATES_FILE):
                        try:
                            with open(DOC_DATES_FILE, "r", encoding="utf-8") as f:
                                return json.load(f)
                        except Exception:
                            pass
                    return {}

                def save_doc_dates(m):
                    os.makedirs(os.path.dirname(DOC_DATES_FILE), exist_ok=True)
                    with open(DOC_DATES_FILE, "w", encoding="utf-8") as f:
                        json.dump(m, f, ensure_ascii=False, indent=2)

                def compute_doc_status(valid_from_str: str, valid_to_str: str) -> str:
                    """
                    Вычисляет статус документа по датам относительно сегодня.
                    Возвращает: "active" | "pending" | "expired" | ""
                    Формат дат: "YYYY-MM-DD" или пустая строка.
                    """
                    today_str = datetime.now().strftime("%Y-%m-%d")
                    has_from = bool(valid_from_str and valid_from_str.strip())
                    has_to   = bool(valid_to_str   and valid_to_str.strip())
                    if not has_from and not has_to:
                        return ""                          # даты не проставлены
                    if has_to and valid_to_str <= today_str:
                        return "expired"                  # утратил силу
                    if has_from and valid_from_str > today_str:
                        return "pending"                  # не вступил в силу
                    return "active"                       # действующая редакция

                def apply_dates_to_index(fname: str, valid_from: str, valid_to: str,
                                         doc_status: str) -> tuple[int, str]:
                    """
                    Обновляет метаданные valid_from / valid_to / doc_status
                    для всех чанков документа в ChromaDB без переиндексации.
                    Возвращает (кол-во обновлённых чанков, сообщение об ошибке).
                    """
                    try:
                        import chromadb as _cdb
                        _vdb_path = os.path.join(
                            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                            "data", "vector_db",
                        )
                        _client = _cdb.PersistentClient(path=_vdb_path)
                        _col    = _client.get_collection(name="tariff_docs")
                        # Получаем ID всех чанков этого файла
                        _res = _col.get(where={"filename": fname}, include=["metadatas"])
                        _ids = _res.get("ids", [])
                        if not _ids:
                            return 0, f"Чанки «{fname}» не найдены в индексе"
                        # Строим патч метаданных — обновляем только три поля
                        _metas = []
                        for _m in _res.get("metadatas", []):
                            _upd = dict(_m or {})
                            _upd["valid_from"]  = valid_from
                            _upd["valid_to"]    = valid_to
                            _upd["doc_status"]  = doc_status
                            # None-значения ChromaDB не принимает — фильтруем
                            _upd = {k: v for k, v in _upd.items() if v is not None}
                            _metas.append(_upd)
                        _col.update(ids=_ids, metadatas=_metas)
                        # Сбрасываем синглтон HybridRetriever — метаданные изменились
                        try:
                ...
                for cat_label, folder in cats_to_show.items():
                    ...
                    for fname in sorted(os.listdir(folder_path)):
                        ...
                        _chroma_info = _chroma_index.get(fname, {})
                        _dates_info  = doc_dates_map.get(fname, {})
                        _vf  = _dates_info.get("valid_from", "")
                        _vt  = _dates_info.get("valid_to",   "")
                        _dst = compute_doc_status(_vf, _vt)
                        all_files.append({
                            "fname":fname,"fpath":fpath,"folder":folder,"cat_label":cat_label,
                            "ext":ext,"size_kb":os.path.getsize(fpath)/1024,
                            "indexed_at":_chroma_info.get("indexed_at","—") if _chroma_info.get("chunks",0)>0 else "—",
                            "chunks_count":_chroma_info.get("chunks",0),"spheres":spheres_map.get(fname,[]),
                            "valid_from":  _vf,
                            "valid_to":    _vt,
                            "doc_status":  _dst,
                        })
                ...
                if not all_files:
                    ...
                    hc = st.columns([1,4,2,3,2,3,1,1,1,1])
                    for col, label in zip(hc, ["Формат","Наименование","Категория","Сферы","Дата индексации","Статус / Даты","📥","🔄","📤","🗑️"]):
                        col.markdown(f"**{label}**")
                    st.divider()
                    EXT_ICONS = {"PDF":"📕","TXT":"📄","DOCX":"📘","XLSX":"📗"}
                    for fi in all_files:
                        row = st.columns([1,4,2,3,2,3,1,1,1,1])
                        ...
                        with row[5]:
                            ...
                            _badge = _STATUS_BADGE.get(fi["doc_status"], "⬜ Даты не заданы")
                            st.caption(_badge)
                            ...
                            if fi["valid_from"]:
                                try:
                                    ...
                                    _vf_val = _date.fromisoformat(fi["valid_from"])
                                except Exception:
                                    pass
                            _new_vf = st.date_input(
                                "Вступает в силу",
                                value=_vf_val,
                                key=f"vf_{fi['fname']}_{fi['folder']}",
                                format="DD.MM.YYYY",
                                help="Дата вступления в силу. Оставьте пустым если неизвестна.",
                            )
                            ...
                            if fi["valid_to"]:
                                try:
                                    from datetime import date as _date
                                    _vt_val = _date.fromisoformat(fi["valid_to"])
                                except Exception:
                                    pass
                            _new_vt = st.date_input(
                                "Утрачивает силу",
                                value=_vt_val,
                                key=f"vt_{fi['fname']}_{fi['folder']}",
                                format="DD.MM.YYYY",
                                help="Дата утраты силы. Оставьте пустым если документ бессрочный.",
                            )

                            # Кнопка «Применить к индексу»
                            if st.button(
                                "💾 Применить",
                                key=f"apply_dates_{fi['fname']}_{fi['folder']}",
                                use_container_width=True,
                                help="Сохранить даты и обновить статус во всех чанках документа",
                            ):
                                _new_vf_str = _new_vf.strftime("%Y-%m-%d") if _new_vf else ""
                                _new_vt_str = _new_vt.strftime("%Y-%m-%d") if _new_vt else ""
                                _new_status = compute_doc_status(_new_vf_str, _new_vt_str)
                                # Сохраняем в JSON
                                doc_dates_map[fi["fname"]] = {
                                    "valid_from": _new_vf_str,
                                    "valid_to":   _new_vt_str,
                                    "doc_status": _new_status,
                                }
                                save_doc_dates(doc_dates_map)
                                # Обновляем метаданные чанков в ChromaDB
                                if fi["chunks_count"] > 0:
                                    _n_upd, _err = apply_dates_to_index(
                                        fi["fname"], _new_vf_str, _new_vt_str, _new_status,
                                    )
                                    if _err:
                                        st.toast(f"❌ {_err}", icon="🚨")
                                    else:
                                        _badge_new = _STATUS_BADGE.get(_new_status, _new_status)
                                        st.toast(
                                            f"✅ {fi['fname']}: {_badge_new} · обновлено {_n_upd} чанков",
                                            icon="💾",
                                        )
                                else:
                                    st.toast(f"💾 Даты сохранены (документ не индексирован)", icon="💾")
                                st.rerun()
        ...
        if tab_chunking.open:
            with tab_chunking:
                ...
                tab4, tab5 = st.tabs(["Параметры чанкования", "Просмотр и тест чанков"], on_change="rerun")
                if tab4.open:
                    with tab4:
        ...
        if tab_predictor.open:
            with tab_predictor:
                show_predictor_tab()



        if tab_claim_rag.open:
            with tab_claim_rag:
